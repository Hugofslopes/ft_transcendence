export async function renderTeamStats(container, team) {
    try {
        ...
        members.sort((a, b) => b.win_rate - a.win_rate);
        const avgWinRate = members.reduce((sum, m) => sum + (m.win_rate ?? 0), 0) / members.length;
        container.innerHTML = `
        ...
        container.innerHTML = `<p>Error loading team stats: ${err.message}</p>`;
    }
}
